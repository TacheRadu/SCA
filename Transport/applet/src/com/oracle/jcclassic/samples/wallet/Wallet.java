...
public class Wallet extends Applet {
    ...
    final static short MAX_LITRES = 500;
    ...
    private void credit(APDU apdu) {
        ...
        if ((moneyCredit > MAX_TRANSACTION_AMOUNT) || (moneyCredit < 0) || (litresCredit > MAX_TRANSACTION_AMOUNT) || (litresCredit < 0)) {
            ISOException.throwIt(SW_INVALID_TRANSACTION_AMOUNT);
        }

        // check the new balance
        if (balance + moneyCredit > MAX_BALANCE || litresCredit + litres > MAX_BALANCE) {
    ...
    private void getBalance(APDU apdu) {
        ...
        if (p == 1) {
            ...
            // returned
            apdu.setOutgoingLength((byte) 2);
        	buffer[0] = (byte) (litres >> 8);
        	buffer[1] = (byte) (litres & 0xFF);
            // send the 2-byte balance at the offset
            // 0 in the apdu buffer
            apdu.sendBytes((short) 0, (short) 2);
        }
            ...
            buffer[0] = (byte) (balance >> 8);
        	buffer[1] = (byte) (balance & 0xFF);
        	buffer[2] = (byte) (litres >> 8);
        	buffer[3] = (byte) (litres & 0xFF);
    ...
    private void resetTryCounter(APDU apdu) {
    	byte[] buffer = apdu.getBuffer();
    	...
    	for (int i = 0; i < byteRead; i++) {
    		if (PUK[(byte)i] != buffer[(byte)i + ISO7816.OFFSET_CDATA]) {
    			ISOException.throwIt(SW_VERIFICATION_FAILED);
    		}
    	}
    	pin.resetAndUnblock();
